...
pub fn render_euro_balance(ui: &mut Ui, _commands_tx: mpsc::Sender<WSCommand>) {
    ...
    let (_balance, has_euro, _euro_line_limit) = euro_rx.borrow().clone();
    ...
    ui.vertical(|ui| {
        if has_euro {
            // Render the EURO balance view if has_euro is true
            euro::render_euro_balance(ui);
        } else {
            // Render an enable button if has_euro is false
            ...
            ui.vertical_centered(|ui| {
                ui.label(RichText::new("Euro Not Enabled").size(30.0).color(text_color));
                ui.add_space(20.0);
                ...
                Frame::NONE
                    ...
                    .show(ui, |ui| {
                        if ui
                            .add(
                                egui::Button::new(RichText::new("Enable Euro").size(button_text_size).color(text_color))
                            ...
                            .clicked()
                        {
                            let _ = xrp_modal_tx.send(XRPModalState {
                                import_wallet: None,
                                create_wallet: None,
                                view_type: ActiveView::EnableEURO,
                            });
