// src/ui/managexrp/rlusd/mod.rs
use egui::{Ui, Color32, Vec2, Pos2, Align2, Area, Frame};
use crate::channel::{CHANNEL, SendRLUSDTransaction, SendRLUSDTransactionState, WSCommand};
use uuid::Uuid;
use tokio::sync::mpsc;
...
pub fn view(ui: &mut Ui, send_state: &mut SendRLUSDTransaction, commands_tx: mpsc::Sender<WSCommand>) -> bool {
    ...
    let (address_buffer, usd_amount_buffer, passphrase_buffer, seed_buffer, input_mode) = get_buffers(&buffer_id);
    ...
    // Create overlay area
    Area::new(egui::Id::new(format!("send_rlusd_overlay_{}", buffer_id)))
        .fixed_pos(pos)
    ...
    // Handle close
    if should_close {
        let _ = send_rlusd_tx.send(SendRLUSDTransactionState { send_rlusd: None });
