// src/ui/managexrp/rlusd/modify/mod.rs
use egui::{Ui, RichText, Area, Pos2, Vec2, Color32, Frame, Align2, Align, Layout};
use crate::channel::{CHANNEL, XRPModalState, ActiveView, WSCommand, ProgressState};
use crate::ui::managexrp::rlusd::shared_utils::{TrustlineStateBuffer, get_or_init_buffer_id, clear_buffer};
use super::modify::buffer_manager::{BufferManager, InputMode};
use super::modify::styles::{get_text_color, modal_fill, modal_stroke, close_button, styled_text_edit, styled_submit_button};
use tokio::sync::mpsc;
...
// Constants
const MODAL_SIZE: Vec2 = Vec2::new(350.0, 190.0);
...
fn get_state(ui: &mut Ui) -> TrustlineStateBuffer {
    ui.data_mut(|d| {
        d.get_persisted(egui::Id::new("modify_trustline_state"))
            .unwrap_or_else(|| {
                let new_state = TrustlineStateBuffer::default();
                d.insert_persisted(egui::Id::new("modify_trustline_state"), new_state.clone());
                new_state
            })
    })
}
...
pub fn validate_and_submit_transaction(
    ...
    if wallet.is_none() {
        ...
        let command = WSCommand {
            ...
            passphrase,
            seed,
            trustline_limit: Some(buffer_manager.trustline_limit().to_string()),
...
pub fn render(ui: &mut Ui, commands_tx: mpsc::Sender<WSCommand>) -> bool {
    ...
    // Overlay
    Area::new(egui::Id::new("modify_overlay"))
        .fixed_pos(pos)
        ...
        .show(ui.ctx(), |ui| {
            ...
            Frame::popup(ui.style())
                ...
                .show(ui, |ui| {
                    ...
                    // Trustline modification UI
                    ui.with_layout(Layout::top_down(egui::Align::Center), |ui| {
                        styled_label(ui, "Modify RLUSD Trustline Limit", TITLE_SIZE, true, text_color);
                        ui.add_space(10.0);
                        ui.add_space(10.0);

                        // Trustline limit slider
 // Trustline limit slider
let mut trustline_limit = buffer_manager.trustline_limit();
ui.with_layout(Layout::top_down(Align::Center), |ui| {
    let available_width = ui.available_width();
    let max_width = (available_width * 0.8).clamp(200.0, 300.0);
    ui.set_max_width(max_width);

    // Estimate slider width (approximating based on typical egui slider size)
    let slider_width = 200.0; // Approximate width, adjustable if needed
    let total_content_width = slider_width;
    let base_padding = (max_width - total_content_width).max(0.0) / 2.0;
    let left_padding = base_padding + 25.0; // Follow radio button logic, using 30.0 offset
    let right_padding = base_padding.max(0.0);

    let trustline_changed = ui.horizontal(|ui| {
        ui.add_space(left_padding); // Push slider to the right with 30.0 offset
        let changed = ui.add(
            egui::Slider::new(&mut trustline_limit, 500_000..=5_000_000)
    ...
    if trustline_changed {
        buffer_manager.update_trustline_limit(trustline_limit);
    }
});
ui.add_space(10.0);
...
ui.add_space(8.0);
    ...
    // Persist state
    ui.data_mut(|d| d.insert_persisted(egui::Id::new("modify_trustline_state"), buffer_manager.to_state_buffer()));

    // Clear buffer and ui.data_mut on close
    if should_close || buffer_manager.done() {
        clear_buffer(&buffer_id);
        ui.data_mut(|d| d.remove::<TrustlineStateBuffer>(egui::Id::new("modify_trustline_state")));
