// src/ui/managexrp/trade/step3.rs
...
pub fn view() -> Element {
    ...
    let on_submit = move |_| {
        ...
        if validation_error.is_none() {
            if base == quote {
                 validation_error = Some("Base and Quote assets cannot be the same.".to_string());
            } else if amount.parse::<f64>().unwrap_or(0.0) <= 0.0 {
                 validation_error = Some("Amount must be greater than zero.".to_string());
            } else if price.parse::<f64>().unwrap_or(0.0) <= 0.0 {
                 validation_error = Some("Limit price must be greater than zero.".to_string());
            }
        }
        ...
        // 4. Spawn Logic
        // We pass the raw strings; calculation happens inside process()
        ...
        // Reset inputs and error state locally
   ...
   rsx! {
        div {
            // border-box ensures the 100% width doesn't cause horizontal overflow
            ...
            // Main Content Area: Relative flow
            div {
                ...
                // --- ACTION BUTTON ---
                // Stays below all items in the vertical flex flow
                button {
