...
pub async fn execute(
    ...
    // No client-initiated execution for get_trustline_euro_limit
    Ok(())
}
    ...
    match message {
        Message::Text(text) => {
            ...
            if let Some(wallet) = data.get("wallet").and_then(|w| w.as_str()) {
                ...
                if command != Some("get_trustline_euro_limit") {
                    return Ok(());
                }

                let (current_euro_balance, _current_has_euro, current_trustline_limit) = CHANNEL.euro_rx.borrow().clone();

                let has_euro = true; // Trustline update implies has_euro is true
                ...
                CHANNEL
                    .euro_tx
                    .send((current_euro_balance, has_euro, Some(trustline_limit)))
