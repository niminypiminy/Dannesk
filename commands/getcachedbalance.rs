...
pub async fn process_response(message: Message, _current_wallet: &str) -> Result<(), String> {
    match message {
        Message::Text(text) => {
            ...
            let xsgd_balance = data.get("xsgd_balance")
                .and_then(|e| e.as_str())
                .and_then(|e| e.parse::<f64>().ok())
                .unwrap_or(0.0);
            ...
            // Extract has_xsgd
            let has_xsgd = data.get("has_xsgd").and_then(|h| h.as_bool()).unwrap_or(false);
            ...
            let trustline_xsgd_limit = data.get("trustline_xsgd_limit")
                .and_then(|l| l.as_str())
                .and_then(|l| l.parse::<f64>().ok());
